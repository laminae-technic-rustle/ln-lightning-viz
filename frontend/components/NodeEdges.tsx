...
const NodeEdges = ({ nodeId }: Props) => {
  const component = React.useMemo<JSX.Element>(() => (<Async url={`/node/${nodeId}/edges`}
        ...
        {edges.map((edge, i) => {
          return (
            <EdgeContainer key={edge.channel_id}>
              ...
              <LabeledElement label="Last Update" value={unixToLocaleDateTime(edge.last_update)}/>
              <LabeledElement label="Channel Id" value={edge.channel_id} />
              <LabeledElement label="Channel Point" value={edge.chan_point} />
              <LabeledElement label="Node 1" value={edge.node1_pub} />
              <LabeledElement label="Node 2" value={edge.node2_pub} />
              <LabeledElement label="Capacity" value={String(edge.capacity)} />
              ...
              */}
              <LabelTitle align="left" fontsize={0.8}>Node 1</LabelTitle>
              {edge.node1_policy ? <NodePolicy policy={edge.node1_policy} /> : <p>No policy</p>} 
              <LabelTitle align="left" fontsize={0.8}>Node 2</LabelTitle>
              {edge.node2_policy ? <NodePolicy policy={edge.node2_policy} /> : <p>No policy</p>}
